  ...
  return (
    <View style={[styles.screen, { backgroundColor: c.bg }]}>
      <View style={[styles.headerBar, { paddingTop: insets.top + 4, borderBottomColor: c.border, backgroundColor: c.card }]}>
        ...
        <Text style={[styles.headerTitle, { color: c.text }]}>Plugins</Text>
        ...
        <Surface style={[styles.group, { backgroundColor: c.card }]} elevation={1}>
          <View style={styles.row}>
            <View style={styles.rowLeft}>
              <Text style={[styles.rowLabel, { color: c.text }]}>Show Plugins Tab</Text>
              <Text style={[styles.rowHint, { color: c.muted }]}>Adds a Plugins tab to the More menu</Text>
            </View>
            <Switch value={showPluginsTab} onValueChange={setShowPluginsTab} />
          </View>
        </Surface>
        <Text style={[styles.hint, { color: c.muted }]}>
          Shows plugins installed in Traefik's static config. View-only - install and remove plugins from the web app.
        </Text>
      </ScrollView>
    </View>
  );
